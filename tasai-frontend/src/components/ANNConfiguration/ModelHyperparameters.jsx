import { FormControl, InputLabel, MenuItem, Select, Checkbox, FormGroup, Typography, Grid } from '@mui/material';
...
export const ModelHyperparameters = () => {
    ...
    const [metrics, setMetrics] = useState([{id:0, chosen:false, title:"fill"}]);
    ...
    const handleMetricCheck = (id) => {
        let mapped = metrics.map(metric => {
            return metric.id == id ? {...metric, chosen: !metric.chosen} : {...metric};
        });
        setMetrics(mapped);
    }
    useEffect(() => {
        ...
        <FormControl fullWidth style={{marginBlock:'20px'}}>
            ...
            <div style={{marginBlock:"20px"}}>
                <Typography variant='h6'>
                    Pasirinkite modelio metrikas
                </Typography>
                <FormGroup>
                    <Grid container spacing={2} justifyContent={"left"}>
                    {metrics.map(metric=>(
                        <Grid item xs={12} md={6} justifyContent={"flex-start"}>
                            <FormControlLabel style={{justifyContent:"left"}} control={<Checkbox style={{justifyContent:"left"}} checked={metric.chosen} onChange={()=>handleMetricCheck(metric.id)}/>} label={metric.title} />
                        </Grid>                
                    ))}
                    </Grid>
                </FormGroup>
